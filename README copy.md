...
* Initial Description and Link: [Corporate Profitability Analysis using the Tidyverse](https://github.com/egabrielvice/FALL2025TIDYVERSE/blob/main/Corporate_Profitability_Analysis.Rmd)
  Dataset: Fortune Top 1000 Companies by Revenue (2022) from Kaggle
  Functions: read_csv(), mutate(), filter(), select(), slice_max(), ggplot2

* Revision/Addition Description and Link: 
  **Extended by Randy Howk**
  - **Employee Efficiency Analysis**: Added revenue and profit per employee calculations to identify which companies generate the most value per worker
  - **Industry-Level Analysis**: Implemented sector-based aggregation using group_by() and summarise() to identify which industries are most profitable and efficient on average
  - **Profitability Tiers**: Created categorical segmentation using case_when() to categorize companies into profitability tiers (Loss Making, Low/Moderate/High/Very High Margin)
  - **Advanced Visualizations**: Added 4 new visualizations including boxplot distribution by sector, scatter plot with trend lines (geom_smooth), and pie chart of profitability distribution
  - **Top Performers by Sector**: Identified industry leaders using slice_max() within group_by()
  - **Comprehensive Summary Statistics**: Added summary statistics including quartiles, standard deviation, and profitability metrics
  - **Functions Extended**: slice_max(), group_by(), summarise(), arrange(), case_when(), count(), geom_boxplot(), geom_smooth(), coord_flip(), coord_polar(), scale_fill_brewer(), ggplot fundamentals
...
* Initial Description and Link: N/A - Focused on EXTEND assignment

* Revision/Addition Description and Link:
  **Extended Escarlet G.'s Corporate Profitability Analysis**
  - Added comprehensive employee efficiency metrics (revenue and profit per employee)
  - Performed industry/sector-level analysis using group_by() and summarise() to identify profitability trends across business sectors
  - Created profitability tier categories using case_when() for company segmentation
  - Developed four additional data visualizations including sector distributions, efficiency trends, and profitability breakdowns
  - Demonstrated advanced dplyr and ggplot2 techniques including faceting, smoothing, and reordering
  - Enhanced analysis depth from basic revenue/profit comparison to multidimensional business efficiency assessment
  - Corporate_Profitability_Analysis.Rmd
